/* "Let's make 16 games in C++ - part 10: BEJEWELED" - by FamTrinli
   ...
   This implementation version is upto 2m46s into the video - the final version

   Implementation by Joseph21, october 6, 2022
   -------------------------------------------

   Fase 4 - delete animation
...
private:
    ...
    struct piece {
        ...
        int match;      // counts the number of matches this piece belongs to in a grid situation
        int alpha;
...
public:
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        // second swap (= swap back) if no match
        if (isSwap && !isMoving) {
            if (score == 0)
                swap( grid[y0][x0], grid[y][x] );
            isSwap = false;
        ...
        for (int i = 1; i <= 8; i++) {
            for (int j = 1; j <= 8; j++) {
                ...
                DrawPartialDecal(
                    ...
                    dGems,
                    olc::vf2d( p.kind * 49.0f,  0.0f ),     // source origin
                    olc::vf2d(          49.0f, 49.0f ),     // source size
                    olc::vf2d(           1.0f,  1.0f ),     // scale (needed to be able to fill in tint parameter)
                    olc::Pixel( 255, 255, 255, p.alpha )    // tint (needed for alpha)
                );
            }
        }
