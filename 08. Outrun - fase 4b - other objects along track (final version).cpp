/* "Let's make 16 games in C++ - part 8: OUTRUN" - by FamTrinli
   ...
   Fase 4b - other objects along the track
...
private:
    struct Line {
        ...
        float X, Y, W;    // screen coordinates (why not int?)
        float scale = 1.0f;
        float curve;
        // used in case an object is associated with a line
        float spriteX = 0.0f;
        float clip = 0.0f;
...
#define   ROAD_COL2    olc::Pixel( 103, 103, 103 )
    ...
    // for background and object decals
    olc::Decal *pBackgrDec = nullptr;
    olc::Decal *pTreeDec   = nullptr;
    olc::Decal *pBushDec   = nullptr;
    olc::Decal *pPitsDec   = nullptr;
...
public:
    bool OnUserCreate() override {
        ...
        int nIgnr, wTree, hTree, wBush, hBush, wPits, hPits;
        pBackgrDec = load_decal_from_file( "./images/bg.png", nIgnr, nIgnr ); if (pBackgrDec == nullptr) std::cout << "ERROR: OnUserCreate() --> couldn't load ./images/bg.png" << std::endl;
        pTreeDec   = load_decal_from_file( "./images/5.png" , wTree, hTree ); if (pTreeDec   == nullptr) std::cout << "ERROR: OnUserCreate() --> couldn't load ./images/5.png"  << std::endl;
        pBushDec   = load_decal_from_file( "./images/6.png" , wBush, hBush ); if (pBushDec   == nullptr) std::cout << "ERROR: OnUserCreate() --> couldn't load ./images/6.png"  << std::endl;
        pPitsDec   = load_decal_from_file( "./images/7.png" , wPits, hPits ); if (pPitsDec   == nullptr) std::cout << "ERROR: OnUserCreate() --> couldn't load ./images/7.png"  << std::endl;

        // fill the screen lines with the distance info - store the depth info for 1600 segments
        // where is this choice of 1600 coming from? I think it's just the length of the track
        for (int i = 0; i < 1600; i++) {
            Line line;
            line.z = i * segL;
            // put curve in this traject
            if (i >  150 && i <  350) line.curve =  0.5f;
            if (i >  450 && i <  700) line.curve = -1.0f;
            if (i >  700 && i <  900) line.curve =  1.0f;
            if (i >  900 && i < 1000) line.curve = -2.0f;
            if (i > 1000 && i < 1100) line.curve =  2.0f;

            // put hills in this traject
            if (i > 1250) line.y = sin( i / 30.0f ) * 1500;

            // put trees, bushes and a pits along the track
            auto put_sprite = [=]( Line &ln, float fx, olc::Decal *decPtr, int nw, int nh ) {
                ln.spriteX   = fx;
                ln.pDec      = decPtr;
                ln.decWidth  = nw;
                ln.decHeight = nh;
            };
            if ((i % 20) == 0 && i < 1250) put_sprite( line, -2.0f, pTreeDec, wTree, hTree );
            if ((i % 20) == 1 && i < 1250) put_sprite( line,  1.0f, pBushDec, wBush, hBush );
            if (i == 690)                  put_sprite( line, -1.5f, pPitsDec, wPits, hPits );
 ...
 */
        ...
        // user controls - use CTRL, SHIFT or SPACE to speed up
        int nMultiplier = 1;
        if (GetKey( olc::Key::CTRL  ).bHeld) nMultiplier =  3;
        if (GetKey( olc::Key::SHIFT ).bHeld) nMultiplier =  7;
        if (GetKey( olc::Key::SPACE ).bHeld) nMultiplier = 15;

        if (GetKey( olc::Key::RIGHT ).bHeld) playerX += nMultiplier * 150;
        if (GetKey( olc::Key::LEFT  ).bHeld) playerX -= nMultiplier * 150;
        if (GetKey( olc::Key::UP    ).bHeld) pos     += nMultiplier * 150;
        if (GetKey( olc::Key::DOWN  ).bHeld) pos     -= nMultiplier * 150;
