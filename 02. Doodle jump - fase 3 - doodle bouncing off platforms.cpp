/* "Let's make 16 games in C++: DOODLE JUMP" - by FamTrinli
   ...
   This implementation version is upto: 1m18s in the video

   Implementation by Joseph21, oktober 1, 2022
   -------------------------------------------

   Fase 3 - doodle bouncing off platforms
...
public:
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        // make doodle bounce on the ground
        dy += 0.2f;
        y += dy;
        if (y > 500.0f) dy = -10.0f;    // use screen size icw doodle size iso 500.0f


        // have the doodle bounce up on the platforms
        for (int i = 0; i < 10; i++) {
