/* "Let's make 16 games in C++: MINESWEEPER" - by FamTrinli
   ...
   This implementation version is upto: 1m34s in the video - the final version
...
public:
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        if (GetMouse( 0 ).bPressed) sgrid[x][y] = grid[x][y];  // open cell
        if (GetMouse( 1 ).bPressed) sgrid[x][y] = 11;          // set flag
        ...
        for (int i = 1; i <= 10; i++) {
            for (int j = 1; j <= 10; j++) {
                // if a mine was left clicked - open up the whole playing board
                if (sgrid[x][y] == 9) {
                    sgrid[i][j] = grid[i][j];
                }
