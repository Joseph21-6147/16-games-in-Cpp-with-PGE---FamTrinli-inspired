...
private:
    ...
    olc::Sprite *frame      = nullptr;

    int field[M][N] = { 0 };  // the playing field (board)

    struct Point {
        int x = 0;            // added initialization of x and y to solve a bug
        int y = 0;
    } a[4], b[4];
...
public:
    bool OnUserCreate() override {
        ...
        background = new olc::Sprite( "images/background.png" );
        frame      = new olc::Sprite( "images/frame.png" );
    ...
    bool OnUserUpdate( float fElapsedTime ) override {
        ...
        // the background fills the entire screen, so clear is not needed anymore
//        Clear( olc::WHITE );
        ...
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                if (field[i][j] != 0) {
                    ...
                    DrawPartialSprite( j * 18 + 28, i * 18 + 31, s, field[i][j] * 18, 0, 18, 18 );
                }
            }
        }
        ...
        for (int i = 0; i < 4; i++) {
            // tile size of the sprite is 18x18
            DrawPartialSprite( a[i].x * 18 + 28, a[i].y * 18 + 31, s, colorNum * 18, 0, 18, 18 );
        }

        // the frame contains transparency - make it work using MASK pixelmode
        SetPixelMode( olc::Pixel::Mode::MASK );
        DrawSprite( 0, 0, frame );
